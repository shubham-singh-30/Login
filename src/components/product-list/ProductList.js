...
const ProductList = () => {
  ...
  const { allProduct, updateProduct } = useSelector(
    (state) => state.cart_reducer
  );
  const [showFilter, setShowFilter] = useState(false);
  const categoryName = useSelector((state) => state.cart_reducer.category);
  const [check, setAllCheck] = useState({
    men: false,
    women: false,
    electronics: false,
    jewelery: false,
  });
  const [filterArr, setFilterArrr] = useState([]);
  ...
  const checkProduct = (e) => {
    ...
    if (checked) {
      let a = updateProduct.filter((item) => item.category === name);
      setFilterArrr([...filterArr, ...a]);
    } else {
      let a = updateProduct.filter((item) => item.category !== name);
      setFilterArrr([...a]);
    }
  ...
  useEffect(() => {
    if (categoryName === "All") {
      ...
      let a = allProduct.filter((item) => item.category === categoryName);
      setProduct(a);
    }
  }, [categoryName]);
  ...
  return (
    <>
      <section
        className="product__container aem-Grid aem-Grid--default--12 aem-Grid--phone--1"
        aria-labelledby="all-products-list"
      >
        <aside className="product__filter aem-GridColumn aem-GridColumn--default--3 aem-GridColumn--tablet--12 aem-GridColumn--phone--1">
          <div className="category">
            <ul>
              ...
              <li className="category_title">
                <p className="product__filter-outwear">{categoryName}</p>
              </li>
            </ul>
          </div>
          ...
          >
            ...
            <div className="filter__item">
              <header>
                <h3 className="filter__item_title" id="all-products-list">
                  Categories
                </h3>
              </header>
        ...
        <aside
          className="product__card_wrapper aem-GridColumn aem-GridColumn--default--9 aem-GridColumn--tablet--12 aem-GridColumn--phone--1"
